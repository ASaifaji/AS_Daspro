...
def tambah(P1,P2):
    if Akar(P2)>=Akar(P1):
        if IsEmpty(Right(P1)):
            return MakePB(Akar(P1),Left(P1),P2)
        else:
            return MakePB(Akar(P1),Left(P1),tambah(Right(P1),P2))
    if Akar(P2)<=Akar(P1):
        if IsEmpty(Left(P1)):
            return MakePB(Akar(P1),P2,Right(P1))
        else:
            return MakePB(Akar(P1),tambah(Left(P1),P2),Right(P1))

def Camouflage(P,X):
    if [X]<=Left(P):
        return [X,[],tambah(Left(P),Right(P))]
    elif [X]>=Left(P):
        return [X,tambah(Left(P),Right(P)),[]]
